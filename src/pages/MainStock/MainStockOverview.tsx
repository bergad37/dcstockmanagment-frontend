...
export default function MainStockOverview() {
  ...
  const totalInStock = products.reduce(
    (sum: number, p: any) => sum + (p.stock?.quantity ?? 0),
    0
  );

  const lowStockItems = products.filter(
    (p: any) => (p.stock?.quantity ?? 0) <= LOW_STOCK_THRESHOLD
  );
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: Warehouse,
      color: 'bg-[#073c56]/10 text-[#073c56]',
    },
    {
    ...
    },
    {
      ...
      sub: `items at or below ${LOW_STOCK_THRESHOLD} units`,
      icon: AlertTriangle,
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex items-center justify-between px-6 py-4 border-b">
            ...
            <a href="/main-stock/stock-in" className="text-xs text-[#073c56] hover:underline font-medium">
              View all →
            </a>
          </div>
                  ...
                  <div key={item.id} className="rounded-xl border border-amber-100 bg-amber-50 p-4">
                    ...
                    <div className="mt-2 w-full bg-amber-100 rounded-full h-1.5">
                      <div
                        ...
                        style={{ width: `${Math.min(100, (qty / LOW_STOCK_THRESHOLD) * 100)}%` }}
                      />
                    </div>
                  </div>
                );
