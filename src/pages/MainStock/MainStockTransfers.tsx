...
const columns = [
    ...
    cell: (row: any) => (
      <div className="py-1">
        ...
        <p className="text-xs text-gray-400">
          {row.mainStockProduct?.category?.name ?? ''}
        </p>
      </div>
    ),
  },
  {
    ...
    cell: (row: any) => (
      <div className="py-1">
        ...
        <p className="text-xs text-gray-400">
          {row.miniStockProduct?.stock?.quantity != null
  ...
  },
  {
    ...
    cell: (row: any) =>
      row.transferredBy ? (
        <span className="text-sm text-gray-700">{row.transferredBy}</span>
      ) : (
...
export default function MainStockTransfers() {
  const { transfers, loading, pagination, fetchTransfers } =
  ...
  // Client-side search filter (product names aren't server-searchable yet)
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="mb-8">
        ...
        <p className="py-2 text-gray-600">
          All movements from Main Stock → Mini Stock
        </p>
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-3 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-lg font-bold text-[#073c56]">
              {pagination?.total ?? transfers.length}
            </p>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-3 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setPage(1);
                load(1, perPage, e.target.value, dateTo);
              }}
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setPage(1);
                load(1, perPage, dateFrom, e.target.value);
              }}
              className="border border-gray-200 rounded-full px-3 py-2 text-sm focus:outline-none focus:border-[#073c56] text-gray-600"
            />
          </div>
          ...
          noDataComponent={
            <div className="py-16 text-center text-gray-400">
              ...
              <p className="text-xs mt-1">
                Use the Transfer button on the Products page to move items to
                Mini Stock.
              </p>
            </div>
          }
